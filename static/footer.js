...
window.Froont.Helpers = (function (window, $) {
  ...
  return {
    ...
    getVimeoIdFromURL: function (url) {
      return _getVideoId(url, ["vimeo.com/", "player.vimeo.com/video/"]);
    },
    ...
    switchToContainerSlide: function (element, slideshowMap, callback, speed) {
      ...
      if (slideIndex || slideIndex === 0) {
        swiperInstance.slideToLoop(
          $parentSlide.data("swiperSlideIndex"),
          speed
        );
        return true;
      }
...
(function ($) {
  ...
  $(function () {
      ...
      $.ajax({
        ...
        success: function () {
          ...
          $.magnificPopup.open(
            {
              items: {
                src: form.querySelector(".js-success-form-submit"),
                type: "inline",
              },
              removalDelay: 300,
        ...
        error: function () {
          ...
          $.magnificPopup.open(
            {
              items: {
                src: form.querySelector(".js-error-form-submit"),
                type: "inline",
              },
              removalDelay: 300,
              mainClass: "mfp-slidein",
    ...
    function onFormSubmit(event) {
      if (this.hasAttribute("data-collect-data")) {
    ...
    $(".linktype-submit").on("click keydown", submitClosestForm);
  });
})(window.jQuery);
window.FRPopup = (function (window, $) {
  ...
  function FRPopup(config) {
    ...
    self.documentScrollTop = 0;
    self.documentScrollLeft = 0;
    self.init();
  }
  FRPopup.prototype = {
    defaults: { selector: ".popup-anchor", closeSelector: ".popup-close" },
    init: function () {
      if (self.$popupEls) {
    ...
    navigateToEl: function (el) {
      ...
      if (elInThisPopup) {
        self.popup.wrap.animate({ scrollTop: $(el).offset().top }, 500);
        return;
      }
    ...
    _recalculateSwiperSlideshow: function ($el) {
      ...
      slideshows.each(function () {
        var swiper =
          window.swiperSlideshows && window.swiperSlideshows[this.id];
        if (swiper) {
          swiper.update(true);
        }
      });
    ...
    _showPopup: function (el, index) {
      var href = el.getAttribute("href"),
        ...
        $prependToEl = null,
        items,
        popupAlign = el.getAttribute("data-popup-align");
      self.showThumbs = false;
      self.closingFromURL = false;
      if (
        $target.length &&
        $target.get(0).classList.contains("popup-gallery")
      ) {
        galleryTarget = $target.get(0);
      }
      if (galleryTarget || el.classList.contains("popup-gallery")) {
        ...
        try {
          ...
          items = items.map(function (item) {
            return { type: "image", src: item.src, title: item.title };
          });
        } catch (e) {
          return;
        }
        ...
        } else if ($target.hasClass("container")) {
          var childWidgets = $target.find(".widget");
          if (!childWidgets.length) {
            var backgroundImage = $target
      ...
      if (!items || (isGallery && !items.length)) {
        return;
      }
      ...
      self.popup.open({
        ...
        showCloseBtn: !el.classList.contains("popup-close-button-hide"),
        index: index,
        autoFocusLast: false,
        fixedContentPos: true,
        gallery: { enabled: isGallery },
        callbacks: {
          beforeOpen: function () {
          ...
          change: function (item) {
            ...
            if (!self.openingFromURL) {
              var urlPart = isGallery
                ? item.index + 1
                : self.locationParts[self.locationParts.length - 1];
              if (isGallery && self.isOpen) {
                self._replaceUrl(
                  "#" +
                    self.locationParts.slice(0, -1).join("/") +
                    "/" +
                    urlPart
                );
              } else {
          ...
          afterClose: function () {
            ...
            requestAnimationFrame(function () {
              window.Froont.Helpers.setPageScrollTop(self.documentScrollTop);
              window.Froont.Helpers.setPageScrollLeft(self.documentScrollLeft);
              var hashFirstPart = document.location.hash
                .replace("#", "")
                .split("/")[0];
              if (
                self.popup._lastFocusedEl &&
                self.locationParts[0] === hashFirstPart
              ) {
    ...
    _showPopupFromURL: function () {
      ...
      self.locationParts = [fragments[0].replace("#", "")].concat(
        fragments.slice(1)
      );
    ...
    _attachThumbnails: function () {
      ...
      var html =
        '<div class="mfp-thumbs-wrap"> <ul class="mfp-thumbs mfp-prevent-close" data-thumbs-count="' +
        mp.items.length +
        '"">';
      mp.items.forEach(function (item, index) {
        html +=
          '<li onclick="javascript:$.magnificPopup.instance.goTo(' +
          index +
          ');return false;"';
        html +=
    ...
    _triggerOpen: function () {
      ...
      if (typeof Event === "function") {
        event = new Event("popup-open");
      } else {
        event = document.createEvent("Event");
        event.initEvent("popup-open", true, true);
      }
    ...
    _triggerClose: function () {
      var event;
      if (typeof Event === "function") {
        event = new Event("popup-close");
      } else {
        event = document.createEvent("Event");
        event.initEvent("popup-close", true, true);
      }
      window.dispatchEvent(event);
    },
  };
...
window.RevealAnimation = (function (window, Waypoint, $) {
  ...
  function RevealAnimation(config) {
    ...
    if (
      (window.Froont.isMobile && !self.config.mobile) ||
      !self._isSupported()
    ) {
      self.destroy();
      return;
  ...
  RevealAnimation.prototype = {
    defaults: {
      mobile: true,
      repeat: true,
      resultClass: "anim-result",
      classPrefix: "animated ",
      animationMarker: ".having-animation",
      animationDataAttr: "animation",
    },
    init: function (config) {
      ...
      var event;
      if (typeof Event === "function") {
        event = new Event("animations-ready");
      } else {
        event = document.createEvent("Event");
        event.initEvent("animations-ready", true, true);
      }
      window.dispatchEvent(event);
      ...
      animation.animationFrame = _requestAnimFrame(function () {
        ...
        $el.on(
          ...
          function (e) {
            if ($el.is(e.target)) {
              $el.removeClass(
                self.config.resultClass + " " + animation.animationClass
              );
              animation.animationFrame = null;
              $el.off(
                "animationend webkitAnimationEnd oAnimationEnd oanimationend MSAnimationEnd"
    ...
    _createAnimationClass: function (element) {
      var animationString = element.getAttribute(
          "data-" + self.config.animationDataAttr
        ),
        animationClass = "";
      if (animationString) {
...
window.FrBgVideos = (function (window, $, Froont) {
  ...
  FrBgVideos.prototype = {
    ...
    _resizeToCover: function (item) {
      ...
      item.videoEl =
        item.videoEl || item.videoContainerEl.querySelector("iframe");
      if (!item.videoEl) {
        return;
      }
    ...
    _getVideoProviderAndId: function (bgVideoUrl) {
      ...
      return { provider: provider, videoId: id };
    },
    ...
    _startAllItems: function () {
      this._items.forEach(function (item) {
        ...
        item.videoContainerEl =
          item.videoContainerEl ||
          $(item.el).find(".bg-video-container").get(0);
        if (!item.videoContainerEl) {
        ...
        if (
          ...
          !item.videoContainerEl.children.length
        ) {
          item.videoContainerEl.innerHTML =
            '<div id="youtube_player_' + item.el.id + '"></div>';
        ...
        if (item.provider === "youtube") {
          ...
          item.player = new YT.Player("youtube_player_" + item.el.id, {
            ...
            events: { onReady: self._onYoutubePlayerReady },
          });
          ...
          item.videoContainerEl.innerHTML =
            ...
            item.videoId +
            "?title=0&byline=0&portrait=0&background=1&loop=1&mute=1&autoplay=1&api=1&player_id=" +
            playerId +
            '" \
    ...
    _findAndRegisterEls: function () {
      $("[data-bg-video-url]").each(function () {
        self.registerEl(this, $(this).data().frBgVideoUrl);
      });
    },
    _addListeners: function () {
      this._debouncedOnResize = window.debounce
        ? window.debounce(this._onResize, 100, false)
        : this._onResize;
...
window.BackgroundParallax = (function (window, $) {
  ...
  BackgroundParallax.prototype = {
    defaults: {
      mobile: false,
      selector: ".background-parallax-active",
      speed: 1,
      boost: 0,
    },
    init: function () {
    ...
    _updateParallax: function () {
      ...
      self.$parallaxEls.each(function (i) {
        ...
        self.parallaxElsData[i].hasOverlayColor =
          bgImg.indexOf("linear-gradient(") !== -1;
        self.parallaxElsData[i].url = bgImgUrl;
        self.parallaxElsData[i].img =
          self.parallaxElsData[i].img || new Image();
    ...
    _readBackgroundImageDimensions: function (i) {
      ...
      var speedAttr = $el.attr("data-background-parallax-speed");
      if (speedAttr) {
        if (speedAttr > 0) {
    ...
    _updateBackgroundPositionOf: function (i) {
      ...
      if (self.parallaxElsData[i].ok && self._isElementInViewport(i)) {
        per =
          (self.scrollY - $el.offset().top + self.screenHeight) /
          ($el.outerHeight() + self.screenHeight);
...
window.Froont.FrGridGallery = (function (window, $) {
  ...
  function FrGridGallery(el, options) {
    this.el = $(el);
    this.options = $.extend({ gallery: { enabled: true } }, options);
    this.gallery = null;
    this.items = [];
    this.init();
  }
  FrGridGallery.prototype = {
    init: function () {
      ...
      if (this.items.length) {
        ...
        var items = this.items.map(function (item) {
          return { type: "image", src: item.image, title: item.title };
        });
        this.items.forEach(function (galleryItem, i) {
          galleryItem.element.addClass("grid-gallery-item");
          galleryItem.element.on(
            "click",
            function (e) {
              e.preventDefault();
              this.gallery.open(
                $.extend(this.options, { items: items, index: i })
              );
            }.bind(this)
          );
        }, this);
    ...
    _getGalleryItems: function (gallery) {
      ...
      gallery.children().each(function (i, gridItem) {
        ...
        gridItem.children().each(function (j, gridItemChildren) {
          ...
          var gridItemChildrenBackground =
            self._getBackgroundImage(gridItemChildren);
    ...
    destroy: function () {
      ...
      if (this.items.length) {
        this.items.forEach(function (galleryItem, i) {
          galleryItem.element.removeClass("grid-gallery-item");
...
jQuery(function ($) {
  ...
  window.Froont.initSlideshow = function (el) {
    var $slideshow = $(el),
      ...
      $slides = $slideWraper.children(".widget.container");
    if ($slides.length && $slideWraper.length === 1) {
      ...
      if (
        typeof mousewheelControl !== "undefined" &&
        mousewheelControl !== "false"
      ) {
        swiperOptions.mousewheel = { releaseOnEdges: true };
        swiperOptions.preventInteractionOnTransition = true;
      }
      var loopDisabled = $slideshow.data("slideshowLoopDisabled");
      loopDisabled = !(
        typeof loopDisabled === "undefined" || loopDisabled === "false"
      );
      if (!loopDisabled) {
      ...
      if (
        typeof touchMoveDisabled !== "undefined" &&
      ...
      var $arrows = $slideWraper.children(".widget.img, .widget.svg"),
  ...
  var onHashChange = function () {
    if (
      ...
      !window.Froont.options.hashChangeByClick
    ) {
      var containerSlide = window.Froont.Helpers.getElementParentSlide(
      ...
      try {
        window.Froont.Helpers.switchToContainerSlide(
          containerSlide,
          window.Froont.swiperSlideshows,
          null,
          0
        );
        $("html,body").scrollTop(
          $(containerSlide).closest(".swiper-container").offset().top
        );
      } catch (e) {}
    }
  ...
  if (window.responsiveNav && $(".navigation-active").length) {
    $(".navigation").each(function () {
      var $navHeader = $(this);
      var $nav = $navHeader.children(".container");
      var $navToggle = $navHeader.children(".navigation-toggle");
      if (!$navToggle.length) {
        $navToggle = $navHeader.children(".svg");
      }
  ...
  $("a:not(.popup-anchor)").on("click", function (event) {
  ...
  if (window.Froont.FrGridGallery) {
    var $gridGalleries = $(".grid-gallery");
    var options = { prependTo: document.querySelector(".b-content") };
    $gridGalleries.each(function (i, gallery) {
      new window.Froont.FrGridGallery(gallery, options);
    });
  }
  var videoElements = $(".video video");
